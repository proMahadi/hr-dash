      ...
      <div className="py-6 px-4 lg:px-9">
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 xl:gap-12 items-center">
          <form className="w-full relative">
            ...
            />
            <SearchIcon className="absolute top-[17px] left-5 z-10 " fill={darkIconStyle}/>
          </form>
          <div className="flex justify-between gap-4 xl:gap-12 items-center">
            <div className="bg-background dark:bg-darkBackground w-fit p-2 flex items-center gap-x-[10px] rounded-lg">
              <button onClick={handleLightMode} className="flex items-center gap-x-2 p-1.5 lg:py-2 lg:px-4  rounded-md bg-primary dark:bg-transparent ">
                <LightIcon fill={isDark==="dark"?"#ffff":"#222222"}/>
                <span className="lg:block hidden text-sm capitalize text-dark dark:text-white">light</span>
              </button>
              <button onClick={handleDarkMode} className="flex items-center gap-x-2 p-1.5 lg:py-2 lg:px-4 rounded-md bg-transparent dark:bg-primary">
                <DarkIcon stroke={isDark==="dark"?"#222222":"#718096"} fill={isDark==="dark"?"#222222":"none"}/>
                <span className="lg:block hidden text-sm capitalize text-textLight dark:text-dark">dark</span>
              </button>
            </div>
            <div className="flex gap-4 xl:gap-7">
              <div className="flex items-center gap-x-4">
                <button className="h-10 w-10 rounded-full border border-textLight dark:border-white flex justify-center items-center relative">
                  ...
                  <span className="h-3 w-3 rounded-full bg-alert text-white text-[6px] flex justify-center items-center absolute top-0 right-0 z-10">3</span>
                </button>
              ...
              <div className="flex items-center gap-3 w-[150px]">
                <div className="text-right">
                  <h6 className="font-medium text-sm text-[#1F2937] dark:text-white">Benjamin</h6>
                  <p className="text-textLight text-xs dark:text-white">Newyork, USA</p>
                </div>
                <div className="flex items-center gap-2">
                  <img src={profileImg} alt="profile-img" className="h-9 w-9 rounded-md" />
